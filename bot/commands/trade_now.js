...
// Robust agent id fetch (DB may have agentID or agentId; session may have it too)
async function getAgentId(ctx, col) {
  const telegramId = String(ctx.from.id);
  if (ctx.session?.agentID) return ctx.session.agentID;
  if (ctx.session?.agentId) return ctx.session.agentId;
  const u = await col.findOne({ telegramId }, { projection: { agentID: 1, agentId: 1 } });
  return u?.agentID || u?.agentId || null;
}
...
module.exports = async function tradeNow(ctx) {
  ...
  try {
    ...
    // 6) Store encrypted strategy on-chain (non-blocking; same UX)
    (async () => {
      try {
        // require contract address configured
        if (!process.env.STRATEGY_STORE_ADDRESS) return;

        const agentId = await getAgentId(ctx, col);
        if (!agentId) {
          console.warn('storeStrategyEncrypted: no agentId in DB/session for', ctx.from.id);
          return;
        }

        // Keep JSON small & non-sensitive
        const strategy = {
          v: 1,
          t: Date.now(),
          source: 'ZkAGI-latest',
